...
> 普通请求放行外网 provider。L1 毫秒级规则先判，灰区/长文本再经 L2 小模型（~230ms）语义二次判定。
...
客户端 (OpenAI SDK / Anthropic / codex / 管理台)
  -> Gateway (FastAPI, :8080)
      ...
      4. L2 小模型 灰区(risk 20~60)/长文本(>30字) 语义二次判定（Qwen3-4B 类小模型，可关）
      5. 策略引擎  policy.yaml 按 priority 命中即停（Pydantic 强校验）
      6. 路由      allow -> 外网 / route_local -> 内网 vLLM / block -> 拦截或降本地
      7. 审计      Redis 热缓存 + MySQL 持久化（90 天），密钥自动脱敏
```
...
- **管理台**（`/admin`，内置前端）：key 登记/启停、规则 CRUD、provider 注册、L2 配置、
  审计查询/CSV 导出、route-inspect 全链路干跑、熔断手动开关

## 快速开始

```bash
# 方式 A：Docker 全栈（无内网模型时先设 OLLAMA_MOCK=true 走 mock）
cp .env.example .env        # 按需填 provider key / 内网地址 / AI_GATEWAY_ADMIN_PASSWORD
docker compose up -d gateway postgres redis
# 已有内网 vLLM：.env 里 OLLAMA_BASE_URL/BGE_BASE_URL/SMALL_MODEL_URL 指过去，OLLAMA_MOCK=false
# 想用容器内 ollama 代替 vLLM：docker compose --profile ollama up -d && docker compose exec ollama ollama pull qwen2:7b

# 方式 B：本地裸跑（mock 内网，零外发）
...
.venv/bin/python -m pytest tests/ -q      # 基线：71 passed + 1 skipped（live 默认 skip）
```

conftest 三件套保证**零真实出境**：`OLLAMA_MOCK=true` + `SMALL_MODEL_ENABLED=false`
+ `routing._mock_enabled` 恒 True（外网 allow 链路也走产品 mock）。文件用例全部内存生成，
OCR 用例双路断言（有引擎走 PII 命中、缺引擎 fail-closed，均不出境）。
...
tests/             pytest 套件（零出境隔离，见上）
static/            演示页 index.html + 管理台前端构建产物（/admin 直服）
...
scripts/           内网 vLLM systemd 模板（按机器改 user/路径/GPU）
